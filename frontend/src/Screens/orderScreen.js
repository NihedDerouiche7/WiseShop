...
const OrderScreen = () => {
    ...
    useEffect(()=>{
        if (!loading) {
            setItemsPrice (  addDecimal(order.orderItems.reduce((acc, item ) => acc + item.price * item.qty , 0)));
  ...
  <Col md={8}>
      ...
      <ListGroup variant="flush">
          <ListGroup.Item>
              <h2>Shipping</h2>
           <p>   <strong style={{color: "red"}}> Name : </strong> {order.user.name}</p>
            
         <p><a href={`mailto:${order.user.email}`} ><strong style={{color: "red"}}>Email :</strong> {order.user.email}</a></p>   
              <p>
                  <strong style={{color: "red"}}>Address : </strong> 
                 {order.shippingAddress.address} ,
                {order.shippingAddress.city}   {order.shippingAddress.postalCode}  </p> 
                <p><strong style={{color: "red"}}>Telephone :</strong> {order.shippingAddress.telephone}</p>
